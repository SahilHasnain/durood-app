...
Next.js admin panel for the `durood-app` repo.
...
`admin/.env.local` should mirror the parent repo’s `durood-app/.env.local`.
...
If those overrides are missing, the admin falls back to the current `durood-app` IDs in [`config.ts`](C:/Users/MD%20SAHIL%20HASNAIN/Desktop/Projects/durood-app/admin/lib/config.ts).
