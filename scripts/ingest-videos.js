...
async function fetchPlaylistItems(playlistId, maxResults = 5000) {
  ...
  for (let i = 0; i < limitedIds.length; i += 50) {
    ...
    const response = await fetch(
      `${baseUrl}/videos?part=contentDetails,snippet,statistics&id=${batchIds.join(",")}&key=${config.youtubeApiKey}`
    );
    ...
    videos.push(
      ...(data.items || []).map((video) => ({
        ...
        views: Number.parseInt(video.statistics?.viewCount || "0", 10) || 0,
        description: video.snippet.description || undefined,
        tags: Array.isArray(video.snippet.tags) ? video.snippet.tags.slice(0, 20) : undefined,
      }))
    );
  }
...
function buildVideoDocument(video, source, sourceName) {
  ...
  const payload = {
    ...
    channelName: source.name || sourceName,
    views: video.views,
...
async function createVideoDocument(databases, video, source, sourceName) {
  return databases.createDocument(
    config.databaseId,
    config.videosCollectionId,
    ID.unique(),
    buildVideoDocument(video, source, sourceName)
  );
}
...
async function ingestSourceVideos(databases, existingMap, source, maxResults, ingestMode) {
  ...
  for (const video of sourceData.videos) {
    try {
      ...
      const ignoreDuration = source.ignoreDuration === true;

      if (!includeShorts && isShort) {
        filteredCount++;
        continue;
      }
      if (!ignoreDuration && ingestMode === "shorts" && !isShort) {
        filteredCount++;
        continue;
      }
      if (!ignoreDuration && ingestMode === "videos" && isShort) {
        filteredCount++;
        continue;
      }
      ...
      await createVideoDocument(databases, video, source, sourceData.sourceName);
...
async function main() {
  try {
    ...
    for (const source of sources) {
      try {
        console.log(`\nProcessing ${source.name || source.youtubeChannelId}...`);
        results.push(
          await ingestSourceVideos(databases, existingMap, source, maxResults, ingestMode)
        );
      } catch (error) {
