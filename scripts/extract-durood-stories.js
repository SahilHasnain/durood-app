...
const CHANNEL_ID = "UCj1jOUBqF1YYpitKF50iL0A";
const UPLOADS_PLAYLIST_ID = `UU${CHANNEL_ID.slice(2)}`;
const TEMP_DIR = path.join(__dirname, "..", "temp-video");
const OUTPUT_FILE = path.join(__dirname, "..", "data", "durood-stories.json");
...
async function main() {
  try {
    ...
    const count = parseInt(process.argv[2], 10) || 2;
    ...
    // Start with existing videos, then append new ones
    const output = {
      channel: "Muslims believe",
    ...
    for (let i = 0; i < toProcess.length; i++) {
      ...
      // Save incrementally every 5 videos
